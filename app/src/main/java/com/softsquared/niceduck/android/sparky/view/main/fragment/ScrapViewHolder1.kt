...
class ScrapViewHolder1(
    ...
    fun bind(item: ScrapDataModel) {
        with(binding) {
            ...
            if (item.imgUrl != null) {
                Glide.with(itemView.context).load(item.imgUrl).centerCrop().into(scrapItem1Img)
            } else {
                Glide.with(itemView.context).load(R.drawable.test_img).centerCrop().into(scrapItem1Img)
            }
